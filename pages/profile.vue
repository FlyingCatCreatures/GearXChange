...
// The types of these are potentially null so we have to done some magic to make everything go smooth
const form = ref({
  email:
    user.value && typeof user.value === "object" && "email" in user.value
      ? (user.value as any).email
      : "",
  password: "",
  confirm: "",
  location:
    user.value && typeof user.value === "object" && "location" in user.value
      ? (user.value as any).location
      : "",
  language:
    user.value && typeof user.value === "object" && "language" in user.value
      ? (user.value as any).language
      : "en",
  name:
    user.value &&
    typeof user.value === "object" &&
    ("name" in user.value || "fullName" in user.value)
...
async function saveProfile() {
  ...
  loading.value = true;
  try {
    const res = $fetch<{ success?: boolean; message?: string }>(
  ...
  <div class="profile-page max-w-xl mx-auto py-10">
    ...
    <form class="space-y-10" @submit.prevent="onSubmit">
      ...
      <div>
        ...
        <div class="mb-4">
          <label class="block mb-1 font-semibold" for="language"
            >Language</label
          >
          <!-- We never got around to actually localising the app but this remains -->
          <select
            id="language"
        ...
        <div class="mb-4">
          <label class="block mb-1 font-semibold" for="location"
            >Location</label
          >
          <input
            id="location"
    ...
    >
      <div class="bg-base-100 p-6 rounded-lg shadow-lg w-full max-w-sm">
        ...
        <div class="flex justify-end gap-2">
          <button class="btn btn-ghost" @click="cancelSaveProfile">
            Cancel
          </button>
          <button class="btn btn-primary" @click="confirmSaveProfile">
            Yes, Save
          </button>
        </div>
      </div>
